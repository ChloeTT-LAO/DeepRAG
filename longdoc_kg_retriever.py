...
class LongDocKGRetriever:
    ...
    def __init__(self, text_model: FlagModel, reranker: FlagReranker, params: Dict):
    ...
    def retrieve(self, query: str, documents: List[Dict], topk: int = None) -> List[Dict]:
        ...
        # 取前topk个
        top_docs = doc_similarities[:topk]
    ...
    def search_with_kg_enhanced_multi_intent(self, query: str, documents: List[Dict]) -> List[Dict]:
        ...
        # ===== 步骤1: 语义检索获取种子chunks =====
        seed_chunks = self.retrieve(query, documents, topk=embedding_topk)
        ...
        if entities and self.kg_processor:
            ...
            for i, refined_query in enumerate(refined_queries):
                ...
                query_results = self.retrieve(refined_query, documents,
                                              topk=embedding_topk // len(refined_queries) + 3)
...
class LongDocKGTester:
    ...
    def _setup_models(self):
        ...
        # 原始检索器（作为基线）
        self.baseline_retriever = LongDocKGRetriever(self.text_model, self.reranker, params)

        # KG增强检索器
        self.kg_enhanced_retriever = LongDocKGRetriever(self.text_model, self.reranker, params)
    ...
    def test_kg_enhancement_with_queries(self, queries: List[str]):
        ...
        for idx, query in enumerate(queries[:self.args.sample_size] if self.args.sample_size > 0 else queries):
            try:
                ...
                # ===== KG增强检索 =====
                kg_start = time.time()
                kg_results = self.kg_enhanced_retriever.search_with_kg_enhanced_multi_intent(
                    query, documents
                )
